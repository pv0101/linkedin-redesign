...
// Consts for modal animation types
const dropIn = {
  hidden: {
    y: "-100vh",
    opacity: 0,
...
// handleClose function from index.js to close modal, type is type of modal
const Modal = ({ handleClose, type }) => {
  const { data: session } = useSession();
  const post = useRecoilValue(getPostState); //get stored post from Post.js through Recoil
