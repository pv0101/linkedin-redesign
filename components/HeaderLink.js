...
//destructure props passed to HeaderLink
function HeaderLink({ Icon, text, avatar, feed, active, hidden }) {
  const { data: session } = useSession();//hook to get user session
  return (
    <div 
    className={`${
      hidden && "hidden md:inline-flex"
        ...
        >
        {/* Styling changes based on whether we are on the feed page or hidden or etc */}
      ...
      {active && ( //underline active icon in header
